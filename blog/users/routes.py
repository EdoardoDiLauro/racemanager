...
def register():
    ...
    if form.validate_on_submit():
        ...
        new_user=User(username=form.username.data,email=form.email.data,password=pass1, idCard=form.id.data)
...
def user_posts(username):
    ...
    travels = Travel.query.filter_by(creator=user).order_by(Travel.date_posted.desc())
    posts = Post.query.filter_by(author=user).order_by(Post.date_posted.desc())
    return render_template('user_posts.html', posts=posts, user=user, travels=travels)
